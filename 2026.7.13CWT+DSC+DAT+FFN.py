...
class DeformableAttentionBlock(nn.Module):
    def __init__(self, dim, num_heads=4, ffn_ratio=4, dropout=0.1):
        ...
        # 前馈神经网络（FFN）
        # 标准做法：Linear -> GELU -> Dropout -> Linear
    ...
    def forward(self, x):
        ...
        attn_out = (attn @ v).reshape(B, self.num_heads, self.head_dim, H, W)
        attn_out = attn_out.permute(0, 2, 1, 3, 4).reshape(B, C, H, W)
        attn_out = self.proj(attn_out)

        # 残差连接，在 HW 维度上做LayerNorm
        ...
        ffn_out = self.ffn(x_flat).reshape(B, H, W, C)

        # 残差 + Norm
        x_perm = x.permute(0, 2, 3, 1)  # [B,H,W,C]
        x_perm = self.norm2(x_perm + ffn_out)

        return x_perm.permute(0, 3, 1, 2)  # [B,C,H,W]
