...
def main():
    ...
    train_indices = torch.where(dataset.split_tags == 0)[0]
    val_indices   = torch.where(dataset.split_tags == 1)[0]
    test_indices  = torch.where(dataset.split_tags == 2)[0]

    def print_type_distribution(name, dataset, indices):
        type_labels = dataset.labels[indices, 0].cpu().numpy()
        counts = np.bincount(type_labels, minlength=4)
        print(f"\n{name} 故障类型分布：")
        print(f"  Normal     (0): {counts[0]}")
        print(f"  Inner Race (1): {counts[1]}")
        print(f"  Ball       (2): {counts[2]}")
        print(f"  Outer Race (3): {counts[3]}")

    train_ds = torch.utils.data.Subset(dataset, train_indices)
    val_ds   = torch.utils.data.Subset(dataset, val_indices)
    test_ds  = torch.utils.data.Subset(dataset, test_indices)

    print_type_distribution("Train", dataset, train_indices)
    print_type_distribution("Val",   dataset, val_indices)
    print_type_distribution("Test",  dataset, test_indices)
