...
class CWRUDataset(Dataset):
    ...
    def __init__(self, root, cache_dir, sample_len=1024, cwt_scales=64, cwt_time_len=64):
        ...
        self.file_map = {
            ...
            "169": (1, 1),  # Inner Race - Medium
            "209": (1, 2),  # Inner Race - Heavy
            "118": (2, 0),  # Ball - Light
            "185": (2, 1),  # Ball - Medium
            "222": (2, 2),  # Ball - Heavy
            "130": (3, 0),  # Outer Race - Light
            "197": (3, 1),  # Outer Race - Medium
            "234": (3, 2),  # Outer Race - Heavy
        }
        self.samples = []
        self.labels = []
        self.source_ids = []
    ...
    def _load_or_cache(self):
        ...
        for mat_name in mats:
            ...
            match_key = None
            for k in sorted(self.file_map.keys(), key=len, reverse=True):
                if k in base:
                    match_key = k
                    break
            ...
            for i in range(n_seg):
                ...
                self.labels.append(torch.tensor([type_label, sev_label], dtype=torch.long))
...
class BearingDiagnosisNet(nn.Module):
    def __init__(self):
        ...
        # 故障类型：Normal / Inner Race / Ball / Outer Race
        self.head_type = nn.Linear(128, 4)

        # 故障程度：Light / Medium / Heavy
...
class EarlyStopping:
    def __init__(self, patience=15, path="best_model.pth", verbose=True):
        ...
        self.counter = 0
        self.best_f1 = -np.inf   # F1 越高越好，初始化为负无穷
    ...
    def __call__(self, val_f1, model):
        if val_f1 > self.best_f1:
            ...
            if self.verbose:
                print(f"  📈 验证F1提升至{val_f1:.4f}，已保存最优权重至 {self.path}")
        else:
            self.counter += 1
            if self.verbose:
            ...
            if self.counter >= self.patience:
                ...
                if self.verbose:
                    print(f"  ⏹️ 触发早停机制 (连续 {self.patience} 轮F1未改善)")
...
def plot_training_curves(history, save_path="training_curves.png"):
    ...
    # --- Type Accuracy ---
    ax = axes[1]
    ax.plot(epochs, history["train_type_acc"], label="Train TypeAcc", color="steelblue")
    ax.plot(epochs, history["val_type_acc"], label="Val TypeAcc", color="tomato")
    ax.plot(epochs, history["val_f1"], label="Val F1 (macro)", color="darkorange", linestyle="--")
    ax.set_title("Fault Type Accuracy & F1")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Score")
    ax.set_ylim(0, 1.05)
    ax.legend()
    ...
    # --- Severity Accuracy ---
    ax = axes[2]
    ax.plot(epochs, history["train_sev_acc"], label="Train SevAcc", color="steelblue")
    ax.plot(epochs, history["val_sev_acc"], label="Val SevAcc", color="tomato")
    ax.set_title("Fault Severity Accuracy")
    ax.set_xlabel("Epoch")
...
def train(model, train_loader, val_loader, device, epochs=100, patience=15, reg_lambda=1e-4, save_dir="."):
    os.makedirs(save_dir, exist_ok=True)

    model_path = os.path.join(save_dir, "best_model.pth")
    curve_path = os.path.join(save_dir, "training_curves.png")

    c_type = torch.nn.CrossEntropyLoss()
    c_sev = torch.nn.CrossEntropyLoss()
    ...
    for epoch in range(1, epochs + 1):
        train_loss, train_type_acc, train_sev_acc = train_epoch(
            model, train_loader, opt, sched,
            c_type, c_sev, device, reg_lambda
        )

        val_loss, val_type_acc, val_sev_acc, val_f1, _, _ = val_epoch(
            model, val_loader, c_type, c_sev, device
        )
        ...
        print(
            ...
            f"| TypeAcc: {val_type_acc:.4f} "
            f"| SevAcc: {val_sev_acc:.4f} "
            f"| ValF1: {val_f1:.4f}"
        )

        # 手动记录最优模型（替代早停），用深拷贝避免被后续训练污染
        if val_f1 > best_f1:
            best_f1 = val_f1
            best_model_state = copy.deepcopy(model.state_dict())

    # 保存最优权重（基于最后一轮的deepcopy快照，不受后续训练影响）
    if best_model_state is not None:
        model.load_state_dict(best_model_state)
        torch.save(best_model_state, model_path)
        print(f"✅ 最优模型已保存 | 最佳 Val F1: {best_f1:.4f}")
...
def main():
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        val_loss, val_type_acc, val_sev_acc, _, _, _ = val_epoch(
            model, val_loader, c_type, c_sev, CONFIG["device"]
        )

        # 仅用于记录曲线，不参与训练、调度和早停
        test_loss, test_type_acc, test_sev_acc, _, _, _ = val_epoch(
        ...
        # 手动记录最优模型（deepcopy 快照，避免被后续训练原地污染）
    ...
    if best_model_state is not None:
        model.load_state_dict(best_model_state)
        torch.save(best_model_state, os.path.join(CONFIG["output_dir"], "best_model.pth"))

    final_val_loss, final_val_type_acc, final_val_sev_acc, _, _, _ = val_epoch(
        model, val_loader, c_type, c_sev, CONFIG["device"]
    )

    final_test_loss, final_test_type_acc, final_test_sev_acc, _, test_preds, test_labels = val_epoch(
        model, test_loader, c_type, c_sev, CONFIG["device"]
    )
    ...
    axes[0].plot(epochs_range, history["val_type_acc"], label="验证集", color="#ff7f0e", linewidth=2,
                 linestyle="--")
    axes[0].plot(epochs_range, history["test_type_acc"], label="测试集", color="#d62728", linewidth=2,
                 linestyle=":")
