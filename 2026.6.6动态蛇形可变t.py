...
class EquivalentFreqFusion(nn.Module):
    def __init__(self, cwt_scales=64):
        super().__init__()
        mask = torch.zeros(1, 1, cwt_scales, cwt_scales)
        fs = 12000
        fault_freqs = [107.3, 74.5, 64.8]
        for f in fault_freqs:
            scale_idx = int(cwt_scales * f / (fs/2))
            if 0 < scale_idx < cwt_scales:
                band = torch.exp(-0.5 * ((torch.arange(cwt_scales) - scale_idx) / 2.0) ** 2)
                mask[0, 0, :, :] += band.unsqueeze(1).repeat(1, cwt_scales)
        self.register_buffer("eff_mask", mask / mask.max())
        self.lambda_w = nn.Parameter(torch.tensor(1.0))

    def forward(self, x):
        mask = F.interpolate(self.eff_mask, size=x.shape[2:], mode='bilinear', align_corners=False)
        return x + self.lambda_w * (x * mask)
...
class BearingDiagnosisNet(nn.Module):
    def __init__(self, cwt_scales=64):
        ...
        self.eff = EquivalentFreqFusion(cwt_scales)
...
def main():
    ...
    model = BearingDiagnosisNet(CONFIG["cwt_scales"]).to(CONFIG["device"])
