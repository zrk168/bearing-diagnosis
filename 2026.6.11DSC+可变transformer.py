...
def train_epoch(model, loader, opt, sched, c_nf, c_type, c_sev, device, reg_lambda):
    ...
    for x, y in loader:
        ...
        loss_nf = c_nf(nf_out, y[:, 0])

        if fault_mask.any():
            loss_type = c_type(type_out[fault_mask], y[fault_mask, 1])
            loss_sev = c_sev(sev_out[fault_mask], y[fault_mask, 2])
        else:
            loss_type = type_out.sum() * 0.0
            loss_sev = sev_out.sum() * 0.0

        loss = loss_nf + loss_type + loss_sev
        ...
        if fault_mask.any():
            c_type_acc += (type_out[fault_mask].argmax(1) == y[fault_mask, 1]).sum().item()
            c_sev_acc += (sev_out[fault_mask].argmax(1) == y[fault_mask, 2]).sum().item()
            fault_n += fault_mask.sum().item()
    ...
    return (
        t_loss / n,
        c_nf_acc / n,
        c_type_acc / max(fault_n, 1),
        c_sev_acc / max(fault_n, 1),
    )
...
def main():
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        t_loss, t_nf, t_type, t_sev = train_epoch(
            model, train_loader, opt, sched,
            c_nf, c_type, c_sev,
            CONFIG["device"],
            model.stability_lambda
        )

        test_loss, test_nf, test_type, test_sev, _, _ = val_epoch(
            model, val_loader,
            c_nf, c_type, c_sev,
            CONFIG["device"]
        )
        ...
        history["train_type_acc"].append(t_type)
        history["test_type_acc"].append(test_type)
        history["train_sev_acc"].append(t_sev)
        history["test_sev_acc"].append(test_sev)
        history["lr"].append(current_lr)

        print(f"Epoch [{epoch:03d}/{CONFIG['epochs']}] | "
              f"Train Loss: {t_loss:.4f} | Test Loss: {test_loss:.4f} | "
              f"Type Acc: Train {t_type * 100:.1f}% / Test {test_type * 100:.1f}% | "
              f"Severity Acc: Train {t_sev * 100:.1f}% / Test {test_sev * 100:.1f}% | "
              f"LR: {current_lr:.2e}")
    ...
    test_loss, test_nf, test_type, test_sev, preds, labels = val_epoch(
        model, val_loader,
        c_nf, c_type, c_sev,
        CONFIG["device"]
    )
    ...
    print(f"🏆 最终测试集性能 | "
          f"Loss: {test_loss:.4f} | "
          f"Type Acc: {test_type * 100:.1f}% | "
          f"Severity Acc: {test_sev * 100:.1f}%")
    ...
    plt.subplot(1, 2, 2)
    plt.plot(epochs_range, history["train_type_acc"], label="Train Acc (Fault Type)", marker="o")
    plt.plot(epochs_range, history["test_type_acc"], label="Test Acc (Fault Type)", marker="s")
    plt.plot(epochs_range, history["train_sev_acc"], label="Train Acc (Severity)", marker="^")
    plt.plot(epochs_range, history["test_sev_acc"], label="Test Acc (Severity)", marker="d")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.ylim(0, 1.05)
    plt.legend()
